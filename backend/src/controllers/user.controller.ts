  ...
  if (avatar) {
    ...
    try {
      const dbUser = await prisma.user.findUnique({
        ...
        select: { avatar: true, isVip: true, vipStartsAt: true, vipExpiresAt: true, isLocked: true, role: { select: { name: true } } },
      });
...
export const uploadAvatar = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const dbUser = await prisma.user.findUnique({
      where: { id: userId },
      select: { isVip: true, vipStartsAt: true, vipExpiresAt: true, isLocked: true, avatar: true, role: { select: { name: true } } },
    });
