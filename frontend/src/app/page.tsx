...
export default function Home() {
  ...
  const [proposed, setProposed] = useState<Movie[]>([]);
  ...
  // Keep the selected anime valid when the API list changes or refreshes.
  useEffect(() => {
    setActiveAnimeIndex((index) => animeList.length ? Math.min(index, animeList.length - 1) : 0);
  }, [animeList.length]);
