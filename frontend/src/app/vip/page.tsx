...
export default function VipPage() {
  ...
  useEffect(() => {
    if (!payosReady || !payosModalOpen || pendingOrder?.provider !== 'PAYOS' || !pendingOrder.checkoutUrl || !window.PayOSCheckout) return;
      ...
      {pendingOrder && (
        <section className="mt-8 overflow-hidden rounded-3xl border border-amber-300/25 bg-gradient-to-r from-amber-400/10 via-slate-950/80 to-slate-950/80 shadow-xl">
          ...
          <div className={`grid gap-7 p-6 lg:items-start ${pendingOrder.provider === 'PAYOS' ? '' : 'lg:grid-cols-[minmax(0,1fr)_320px]'}`}>
            ...
            {pendingOrder.provider === 'PAYOS' ? null : paymentQrUrl ? <div className="mx-auto w-full max-w-[320px] rounded-3xl border border-white/10 bg-white p-3 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
