...
export default function WatchTogetherPage() {
  ...
  const source = movie?.episodes?.find((item: any) => item.episodeOrder === episode)?.videoSources?.[0]; const isHost = hostId === socketRef.current?.id;
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => { setConnected(true); const event = requestedRoom ? 'room:join' : 'room:create'; socket.emit(event, { roomId: requestedRoom, slug, episode, name: name || user?.username }, (result: any) => { if (result?.error) { setError(result.error); setStarted(false); return; } setRoomId(result.roomId); setState(result.state); setUsers(result.users || []); setHostId(result.hostId || ''); window.history.replaceState({}, '', `/watch-together?room=${result.roomId}&slug=${encodeURIComponent(result.slug)}&ep=${result.episode}`); }); });
    socket.on('room:state', (next: RoomState) => setState(next));
    socket.on('room:users', (snapshot: { users: RoomUser[]; hostId: string }) => { setUsers(snapshot.users || []); setHostId(snapshot.hostId || ''); }); socket.on('room:message', (next: ChatMessage) => setMessages((current) => [...current.slice(-49), next])); socket.on('connect_error', () => setError('Không kết nối được máy chủ xem chung.'));
