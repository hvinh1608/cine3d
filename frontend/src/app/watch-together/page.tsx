...
export default function WatchTogetherPage() {
  ...
  const [state, setState] = useState<RoomState>({ playing: false, currentTime: 0, updatedAt: Date.now() }); const [users, setUsers] = useState<RoomUser[]>([]); const [messages, setMessages] = useState<ChatMessage[]>([]); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [started, setStarted] = useState(Boolean(requestedRoom)); const [connected, setConnected] = useState(false); const [soundEnabled, setSoundEnabled] = useState(false);
