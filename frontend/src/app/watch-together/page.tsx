...
export default function WatchTogetherPage() {
  ...
  const { user } = useStore(); const [name, setName] = useState(user?.username || ''); const [roomId, setRoomId] = useState(initialRoomId); const [movie, setMovie] = useState<any>(null);
  const [state, setState] = useState<RoomState>({ playing: false, currentTime: 0, updatedAt: Date.now() }); const [users, setUsers] = useState<RoomUser[]>([]); const [hostId, setHostId] = useState(''); const [messages, setMessages] = useState<ChatMessage[]>([]); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [started, setStarted] = useState(Boolean(initialRoomId)); const [connected, setConnected] = useState(false); const [soundEnabled, setSoundEnabled] = useState(false); const [playbackBlocked, setPlaybackBlocked] = useState(false);
  const socketRef = useRef<Socket | null>(null); const videoRef = useRef<HTMLVideoElement>(null); const hlsRef = useRef<Hls | null>(null); const remoteUpdate = useRef(false); const pendingSeekHandler = useRef<(() => void) | null>(null);
  ...
  const applyGuestPlayback = useCallback((next: RoomState) => {
    ...
    const applyPlaying = () => {
      ...
      void video.play()
        ...
        .finally(() => { remoteUpdate.current = false; });
    };

    if (Math.abs(video.currentTime - next.currentTime) > 1.25) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      socketRef.current.emit('room:control', { type: video.paused ? 'pause' : 'play', currentTime: video.currentTime });
    }, 3_000);
    return () => window.clearInterval(interval);
  }, [connected, isHost]);
  useEffect(() => {
    if (!started) return; const socket = io(SOCKET_URL, { withCredentials: true }); socketRef.current = socket;
    socket.on('connect', () => { setConnected(true); const event = initialRoomId ? 'room:join' : 'room:create'; socket.emit(event, { roomId: initialRoomId, slug, episode, name: name || 'Khách' }, (result: any) => { if (result?.error) { setError(result.error); setStarted(false); return; } setRoomId(result.roomId); setState(result.state); setUsers(result.users || []); setHostId(result.hostId || (!initialRoomId ? socket.id : '')); window.history.replaceState({}, '', `/watch-together?room=${result.roomId}&slug=${encodeURIComponent(result.slug)}&ep=${result.episode}`); }); });
  ...
  }, [started, initialRoomId, slug, episode, name]);

  const control = (type: 'play' | 'pause' | 'seek') => { const video = videoRef.current; const currentTime = video?.currentTime || 0; const playing = type === 'seek' ? Boolean(video && !video.paused) : type === 'play'; setState({ playing, currentTime, updatedAt: Date.now() }); socketRef.current?.emit('room:control', { type, currentTime }); };
