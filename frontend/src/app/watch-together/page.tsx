...
export default function WatchTogetherPage() {
  ...
  const [state, setState] = useState<RoomState>({ playing: false, currentTime: 0, updatedAt: Date.now() }); const [users, setUsers] = useState<RoomUser[]>([]); const [messages, setMessages] = useState<ChatMessage[]>([]); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [started, setStarted] = useState(false); const [connected, setConnected] = useState(false);
  ...
  useEffect(() => { if (!source || !videoRef.current) return; const video = videoRef.current; hlsRef.current?.destroy(); if (source.type === 'hls' && Hls.isSupported()) { const hls = new Hls(); hls.loadSource(source.url); hls.attachMedia(video); hlsRef.current = hls; } else video.src = source.url; return () => { hlsRef.current?.destroy(); hlsRef.current = null; }; }, [source]);
  useEffect(() => {
    ...
    socket.on('room:state', (next: RoomState) => { setState(next); const video = videoRef.current; if (!video) return; remoteUpdate.current = true; video.currentTime = next.currentTime; (next.playing ? video.play() : Promise.resolve(video.pause())).finally(() => { remoteUpdate.current = false; }); });
