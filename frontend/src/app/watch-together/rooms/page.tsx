...
export default function WatchTogetherRoomsPage() {
  ...
  if (!user) return <main className="mx-auto flex min-h-[70vh] max-w-lg items-center px-4"><div className="w-full rounded-[28px] border border-white/10 bg-[#252832] p-8 text-center shadow-2xl"><Users className="mx-auto h-12 w-12 text-amber-300" /><h1 className="mt-4 text-2xl font-black">Đăng nhập để xem chung</h1><p className="mt-2 text-sm text-slate-400">Danh sách phòng và tính năng xem chung chỉ dành cho thành viên.</p><Link href="/account" className="mt-6 block rounded-xl bg-amber-300 py-3 text-sm font-black text-slate-950">Đăng nhập / Đăng ký</Link></div></main>;

  return <main className="min-h-screen pb-20 text-white">
    <section className="relative isolate overflow-hidden border-b border-white/5">
    <div className="absolute inset-0 -z-20 bg-[#191a22]" />
    <div className="absolute inset-0 -z-10 opacity-90" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(250,204,21,.19), transparent 55%), linear-gradient(180deg, rgba(42,44,55,.6), #191a22)' }} />
    <div className="mx-auto flex min-h-[330px] max-w-[1500px] flex-col items-center justify-center px-4 text-center">
      <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-amber-300/25 bg-amber-300/10 px-3 py-1 text-xs font-bold text-amber-200"><Radio className="h-3.5 w-3.5 animate-pulse" /> CÔNG CHIẾU CINE3D</span>
      ...
      <Link href="/search" className="inline-flex items-center gap-2 rounded-full bg-amber-300 px-6 py-3 text-sm font-black text-slate-950 shadow-lg shadow-amber-950/20 transition hover:bg-amber-200"><CirclePlus className="h-4 w-4" /> Tạo phòng mới</Link>
        </div>
      </div>
    </section>
  ...
  <section className="mx-auto max-w-[1500px] px-4 py-10 md:px-8">
      ...
      {visibleRooms.length ? <div className="grid gap-x-5 gap-y-8 sm:grid-cols-2 xl:grid-cols-4">
        {visibleRooms.map((room) => {
          ...
          <div className="relative aspect-video overflow-hidden rounded-[22px] border border-white/10 bg-[#252832] shadow-lg transition duration-300 group-hover:-translate-y-1 group-hover:border-amber-300/40 group-hover:shadow-black/40">
        ...
        <div className="mt-3 flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 border-amber-300 bg-gradient-to-br from-amber-300/30 to-orange-500/20 text-xs font-black">{room.hostName.charAt(0).toUpperCase()}</div><div className="min-w-0"><h3 className="truncate text-sm font-black text-white group-hover:text-amber-200">Cùng xem {movie?.title || room.slug}</h3><p className="mt-0.5 truncate text-xs text-slate-400">Tập {room.episode} · {room.hostName}</p><p className="mt-1 flex items-center gap-1 text-[10px] text-slate-600"><Clock3 className="h-3 w-3" /> {timeAgo(room.createdAt)}</p></div></div>
          </Link>;
        })}
